...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        if self.current_state.id == 'heat':
            if temp < self.setPoint:
                redLight.pulse()  # pulse redLight is heat mode turned on and below set temp
            else:
                redLight.on() # Solid redLight if set point reached or above temp
        elif self.current_state.id == 'cool':
            if temp > self.setPoint:
                blueLight.pulse() #pulse blue light if cool mode and above set temp
            else:
                blueLight.on() # solid blue light if at/below set temp
    ##
    ...
    def setupSerialOutput(self):
        ...
        output = f"{self.current_state.id}, {temp}F, {point}F"   
        return output
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                ## degrees Fahrenheit. 
                lcd_line_2 = f"Temp: {round(self.getFahrenheit(), 1)}F" 
                altCounter = altCounter + 1
            else:
                ##
                ## second line of the LCD display to incude the current state of the 
            ...
            if((counter % 30) == 0):
                ##
                ## send our current state information to the TemperatureServer
